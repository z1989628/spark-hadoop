...
public class Dataset {
	...
	// Converts a token to its corresponding integer for a given attribute
	...
	// return number of attributes that are not IGNORE
	...
	private static void validateValues(Attribute[] attrs, List<String>[] values) {
		...
		for (int attr = 0; attr < attrs.length; attr++) {
			Preconditions.checkArgument(!attrs[attr].isCategorical() || values[attr] != null, "value not found for attribute " + attr);
		}
	}
	...
	public static Dataset load(Configuration conf, Path path) throws IOException {
	  ...
	  // deserialize an instance from a string
	...
	private Map<String, Object> getMap(Attribute type, String[] values,
			boolean isLabel) {
