...
// This class can be used when the criterion variance is the categorical attribute.
public class OptIgSplit extends IgSplit {
	...
	// Computes the split for CATEGORICAL attribute
	...
	static Split numericalSplit(Data data, int attr) {
		...
		for (int index = 0; index < splitPoints.length; index++) {
			...
			// instance with attribute value >= value[index]
		...
		if (best == -1) {
			throw new IllegalStateException("no best split fount !");
		}
