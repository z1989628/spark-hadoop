...
import randomForest.df.node.CategoricalNode;
import randomForest.df.node.Leaf;
import randomForest.df.node.Node;
import randomForest.df.node.NumericalNode;
import randomForest.df.split.IgSplit;
import randomForest.df.split.OptIgSplit;
import randomForest.df.split.RegressionSplit;
import randomForest.df.split.Split;

//Builds a classification tree or regression tree
//A classification tree is built when the criterion variable is categorical attribute
...
public class DecisionTreeBuilder implements TreeBuilder {
	...
	public void setMinVarinaceProportion(double minVarianceProportion) {
	...
	public Node build(Random rng, Data data) {
		...
		log.debug("best split attr:{}, split:{}, ig:{}", best.getSplit(), best.getIg());
		...
		if (data.getDataset().isNumerical(best.getAttr())) {
			...
			if (loSubset.isEmpty() || hiSubset.isEmpty()) {
				// the selected attribute did not change the data, avoid using the child nodes
				selected[best.getAttr()] = true;
			} else {
				// the data changed, so we can unselect all previously selected NUMERICAL attributes
				temp = selected;
				selected = cloneCategoricalAttributes(data.getDataset(), selected);
			}
			
			// size of the subset is less than the minSplitNum
	...
	// checks if all vectors have identical attribute values. Ignore selected attributes;
