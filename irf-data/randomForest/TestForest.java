...
import randomForest.classifier.RegressionResultAnalyzer;
import randomForest.classifier.ResultAnalyzer;
...
import randomForest.df.mapreduce.Classifier;
...
public class TestForest extends Configured implements Tool {
	...
	private Path outputPath; // path to predictions file, if null do not output
								// the predictions
	...
	public int run(String[] args) throws IOException, ClassNotFoundException,
		...
		Option datasetOpt = obuilder
				...
				.withRequired(true)
				.withArgument(
						abuilder.withName("dataset").withMinimum(1)
		...
		Option mrOpt = obuilder.withLongName("mapreduce").withShortName("mr")
				.withRequired(false).create();

		Option helpOpt = DefaultOptionCreator.helpOption();

		Group group = gbuilder.withName("Options").withOption(inputOpt)
				.withOption(datasetOpt).withOption(modelOpt)
				.withOption(outputOpt).withOption(analyzeOpt).withOption(mrOpt)
		...
		try {
			...
			String outputName = cmdLine.hasOption(outputOpt) ? cmdLine
					.getValue(outputOpt).toString() : null;
	...
	private void testForest() throws IOException, ClassNotFoundException,
			InterruptedException {
	...
	private void mapreduce() throws ClassNotFoundException, IOException,
			InterruptedException {
		if (outputPath == null) {
			throw new IllegalArgumentException(
		...
		Classifier classifier = new Classifier(modelPath, dataPath,
				datasetPath, outputPath, getConf());
			...
			if (results != null) {
				...
				if (dataset.isNumerical(dataset.getLabelId())) {
					...
					ResultAnalyzer analyzer = new ResultAnalyzer(
							Arrays.asList(dataset.labels()), "unknown");
					for (double[] res : results) {
						analyzer.addInstance(
								dataset.getLabelString(res[0]),
								new ClassifierResult(dataset
										.getLabelString(res[1]), 1.0));
					}
					log.info("{}", analyzer);
				}
			}
	...
	private void sequential() throws IOException {
		...
		if (dataFS.getFileStatus(dataPath).isDir()) {
			...
			testFile(dataPath, outputPath, converter, forest, dataset, resList,
					rng);
		}
			...
			if (dataset.isNumerical(dataset.getLabelId())) {
				...
				for (double[] r : resList) {
					analyzer.addInstance(dataset.getLabelString(r[0]),
							new ClassifierResult(dataset.getLabelString(r[1]),
									1.0));
				}
				log.info("{}", analyzer);
			}
